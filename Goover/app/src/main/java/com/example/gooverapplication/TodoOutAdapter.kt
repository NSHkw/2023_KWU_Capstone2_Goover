...
class TodoOutAdapter(val context: Context, val itemList: MutableList<RecyclerOutViewModel>)
    : RecyclerView.Adapter<TodoOutAdapter.Holder>()
{
    ...
    inner class Holder(var binding: ItemRecyclerviewWrongOuterBinding) :
        ...
        fun bind(item: RecyclerOutViewModel) {
            binding.model = item
            binding.innerRecyclerView.adapter = TodoInAdapter(context, item.innerList)
            binding.innerRecyclerView.layoutManager = LinearLayoutManager(context)
            binding.innerRecyclerView.addItemDecoration(TodoDecoration(context))
